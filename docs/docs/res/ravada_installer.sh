...
if [[ $OS = *"Ubuntu"* && $VER > '16.04' ]]; then
  ...
  sudo dpkg -i ravada_0.6.0_ubuntu-18.04_all.deb &> /dev/null
  ...
  wget http://infoteleco.upc.edu/img/debian/ravada_0.6.0_all.deb &> /dev/null
  sudo dpkg -i ravada_0.6.0_all.deb &> /dev/null
  echo "Installing Dependencies..."
  sudo apt-get update -y &> /dev/null
  sudo apt-get -f -y install &> /dev/null
