...
sub init_config {
    ...
    # TODO list all and pick the one with more available space
    my $default_dir_kvm = '';#"/var/lib/libvirt";
    for my $pool ($VM->list_storage_pools) {
        my $doc = $PARSER->load_xml(string => $pool->get_xml_description);

        my ($path) =$doc->findnodes('/pool/target/path/text()');
        next if !$path;
        $default_dir_kvm = $path;
        $VM_POOL = $pool;
    }
    die "I can't find /pool/target/path in the storage pools xml\n"
        if !$default_dir_kvm;

    $CONFIG->{kvm}->{dir} = $default_dir_kvm
        if !exists $CONFIG->{kvm}->{dir}
}
...
sub check_req_new_base{
        my $sth = $CON->dbh->prepare(
            "SELECT * "
            ." FROM bases_req "
            ."    WHERE isnull(error) AND created='n' "
        );
        $sth->execute;
        while (my $row = $sth->fetchrow_hashref) {
            ...
            warn Dumper($row);
            my $uri;
            eval { $uri=kvm_create_base($row) };
            if ($@) {
                req_base_error($row->{id}, $@);
            } else {
                my $sth2 = $CON->dbh->prepare(
                    "UPDATE bases_req set uri =?,created='y' "
                    ." WHERE id=?"
                );
                $sth2->execute($uri , $row->{id});
                $sth2->finish;
                warn "Created base $row->{name}";
            }
        }
...
sub req_base_error {
    my ($id, $error) = @_;
    my $sth2 = $CON->dbh->prepare(
                    "UPDATE bases_req set uri = NULL, error = ? "
                    ." WHERE id=?"
    );
    $sth2->execute($error, $id);
    $sth2->finish;
...
sub create_qcow_base {
    ...
    my $dir_img = $CONFIG->{kvm}->{dir};# || $DEFAULT_DIR_IMG);

    die "Missing $dir_img\n"    if ! -e $dir_img;

    my $qcow_img = "$dir_img/$base_name.ro.qcow2";
...
sub vm_vol_remove {
    ...
    my $vol = $VM_POOL->get_volume_by_name($name);
    if (!$vol) {
...
sub domain_remove_db {
    my $name = shift;
    my $sth = $CON->dbh->prepare("DELETE FROM domains WHERE name=?");
    $sth->execute($name);
    $sth->finish;
}
...
sub domain_remove {
    my $name = shift;
    domain_remove_kvm($name);
    domain_remove_db($name);
}

sub add_user {
    my $login = shift;
    Ravada::Auth::SQL::init($CONFIG, $CON);
...
sub select_iso {
    my $sth = $CON->dbh->prepare("SELECT * FROM iso_images ORDER BY name");
    $sth->execute;
    my @isos;
    while (my $row = $sth->fetchrow_hashref) {
        lock_hash(%$row);
...
sub search_iso {
    my $id_iso = shift;
    my $sth = $CON->dbh->prepare("SELECT * FROM iso_images WHERE id = ?");
    $sth->execute($id_iso);
...
sub kvm_create_vol {
    ...
    die "Missing volume name"   if !$name;
    eval { vm_vol_remove("$name.img") };
    my $file_xml = "etc/xml/$iso->{xml_volume}";
    ...
    my $vol = $VM_POOL->create_volume($doc->toString);
...
sub iso_name {
    ...
    my $device = "$CONFIG->{kvm}->{dir}/$iso_name";
